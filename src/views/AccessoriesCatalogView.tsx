...
import { Sparkles, Star, ShoppingBag, Filter, Heart, Eye, Check, Search, Award } from 'lucide-react';
...
export const AccessoriesCatalogView: React.FC = () => {
  ...
  const categories = [
    { id: 'all', label: 'All Accessories', icon: '✨' },
    { id: 'collars', label: 'Collars', icon: '📿' },
    { id: 'leashes', label: 'Leashes & Leads', icon: '🪢' },
    { id: 'harnesses', label: 'Harnesses', icon: '🦺' },
    { id: 'beds', label: 'Beds & Mats', icon: '🛋️' },
    { id: 'toys', label: 'Chew & Plush Toys', icon: '🎾' },
    { id: 'bowls', label: 'Bowls & Feeders', icon: '🥣' },
    { id: 'grooming-essentials', label: 'Grooming Tools', icon: '🧼' },
    { id: 'travel', label: 'Travel & Carriers', icon: '🚗' },
  ];

  const brands = ['all', 'Trixie', 'Pethinc', 'Coastal Tails', 'Kong', 'Pet Bed Co.', 'Wahl'];

  const filteredProducts = useMemo(() => {
    let prods = [...ACCESSORY_PRODUCTS];
  ...
  return (
    <div className="py-8 sm:py-12 bg-[#FDFBF7] animate-fadeIn min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        {/* Page Header */}
        <div className="text-center max-w-3xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#EFE8DB] text-[#08383B] text-xs font-bold uppercase tracking-wider">
            <Sparkles className="w-3.5 h-3.5 text-[#0D6E6E]" />
            <span>PREMIUM COASTAL PET ACCESSORIES</span>
          </div>
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#08383B] font-['Outfit']">
            Shop Pet Accessories & Gear
          </h1>
          <p className="text-sm sm:text-base text-slate-600">
            Handcrafted leather collars, no-pull breathable harnesses, waterproof beach leashes, calming orthopedic beds, and interactive toys.
          </p>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              placeholder="Search collars, beds, harnesses, toys..."
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-3xl border border-slate-200 p-8 space-y-3">
            <p className="text-base font-bold text-slate-800">No accessories match your filters</p>
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-[#08383B] text-white text-xs font-bold"
            >
              Reset Filters
            </button>
          </div>
